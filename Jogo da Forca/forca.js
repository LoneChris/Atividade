...
function atualizarInterface() {
    ...
    document.getElementById("visual-vidas").innerText = "♡".repeat(vidas) + "X".repeat(5 - vidas);
...
function jogarAposta() {
    ...
    if (palpite === jogoAtual.palavra) {
        ...
        atualizarInterface();
        finalizarJogo(true, "Incrivel! Você acertou a Palavra Completa!");
    } else {
        // Se errar a aposta final, perde todas as vidas imediatamente
        vidas = 0;
        atualizarInterface();
        finalizarJogo(false, `Resposta errada. A palavra era: ${jogoAtual.palavra}`);
    }
}

function verificarVitoria() {
    if (!letrasDescobertas.includes("_")) {
        finalizarJogo(true, "Incrivel! Você descobriu a palavra!");
    }
}

function verificarDerrota() {
    if (vidas <= 0) {
        finalizarJogo(false, `Você perdeu. A palavra era: ${jogoAtual.palavra}`);
    }
}
